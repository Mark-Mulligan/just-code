...
const largestNumInArrayTests = () => {
  ...
  testResults.push({
    ...
    passed:
      hasThreeRepeatingValues(['repeat', 'repeats', 'repeat', 'repeating', 'repeats', 'repeating', 'repeat']) === true,
    result: JSON.stringify(
      hasThreeRepeatingValues(['repeat', 'repeats', 'repeat', 'repeating', 'repeats', 'repeating', 'repeat']),
    ),
  });
  testResults.push({
    test: 'hasThreeRepeatingValues(["repeat", "repeats", "repeat", "repeating", "repeats", "repeated"]) returns false',
    passed: hasThreeRepeatingValues(['repeat', 'repeats', 'repeat', 'repeating', 'repeats', 'repeated']) === false,
    result: JSON.stringify(
      hasThreeRepeatingValues(['repeat', 'repeats', 'repeat', 'repeating', 'repeats', 'repeated']),
    ),
  });
  testResults.push({
