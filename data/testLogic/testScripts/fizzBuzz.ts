...
const checkArrContents = (inputArr: (string | number)[]) => {
  ...
  if (!Array.isArray(inputArr)) {
    ...
    return { passed: false, message: 'Return array did not contain 100 values' };
  }
  ...
  if (hasNumbers && hasStrings) {
    return { passed: true, message: 'Funtion returns an array containing 100 values with numbers and strings.' };
  } else if (!hasNumbers) {
    ...
    return { passed: false, message: 'Return array does not have numbers or strings.' };
  }
};
...
export const fizzBuzzTests = () => {
  ...
  testResults.push({
    test: 'The returned array contains numbers and strings and is contains 100 values.',
...
export const fizzBuzzTestScript = createTestScriptString(fizzBuzzTests, [
...
export const fizzBuzzTestCriteria = extractTestCriteria(fizzBuzzTests());
